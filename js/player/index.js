...
export default class Player extends Animation {
   ...
   */
  shoot() {
    const bullet = globalThis.databus.pool.getItemByClass('bullet', Bullet);
    bullet.init(this.x + this.width / 2 - bullet.width / 2, this.y - 10, 10);
    globalThis.databus.bullets.push(bullet);
    globalThis.musicManager.playShoot(); // 播放射击音效
  }
  ...
  update() {
    ...
    // 每20帧让玩家射击一次
    if (globalThis.databus.frame % PLAYER_SHOOT_INTERVAL === 0) {
      this.shoot(); // 玩家射击
    }
  }
  ...
  destroy() {
    this.isActive = false;
    this.playAnimation();
    globalThis.musicManager.playExplosion(); // 播放爆炸音效
