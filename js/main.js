...
globalThis.musicManager = new Music(); // 全局音乐管理实例
...
export default class Main {
  ...
  enemyGenerate() {
    // 每30帧生成一个敌机
    if (globalThis.databus.frame % ENEMY_GENERATE_INTERVAL === 0) {
      const enemy = globalThis.databus.pool.getItemByClass('enemy', Enemy); // 从对象池获取敌机实例
      enemy.init(); // 初始化敌机
      globalThis.databus.enemys.push(enemy); // 将敌机添加到敌机数组中
    }
  }
  ...
  collisionDetection() {
    // 检测子弹与敌机的碰撞
    globalThis.databus.bullets.forEach((bullet) => {
      for (let i = 0, il = globalThis.databus.enemys.length; i < il; i++) {
        const enemy = globalThis.databus.enemys[i];

        // 如果敌机存活并且发生了发生碰撞
        if (enemy.isCollideWith(bullet)) {
          enemy.destroy(); // 销毁敌机
          bullet.destroy(); // 销毁子弹
          globalThis.databus.score += 1; // 增加分数
          break; // 退出循环
        }
      }
    });
    ...
    for (let i = 0, il = globalThis.databus.enemys.length; i < il; i++) {
      ...
      if (this.player.isCollideWith(enemy)) {
        ...
        globalThis.databus.gameOver(); // 游戏结束
  ...
  render() {
    ...
    globalThis.databus.bullets.forEach((item) => item.render(ctx)); // 绘制所有子弹
    globalThis.databus.enemys.forEach((item) => item.render(ctx)); // 绘制所有敌机
    this.gameInfo.render(ctx); // 绘制游戏UI
    globalThis.databus.animations.forEach((ani) => {
      if (ani.isPlaying) {
  ...
  update() {
    ...
    // 更新所有子弹
    globalThis.databus.bullets.forEach((item) => item.update());
    // 更新所有敌机
    globalThis.databus.enemys.forEach((item) => item.update());
