...
const atlas = adapter.createImage();
...
export default class GameInfo extends Emitter {
  constructor() {
    ...
    // 绑定触摸事件
    adapter.onTouchStart(this.touchEventHandler.bind(this))
  }
  ...
  render(ctx) {
    this.renderGameScore(ctx, globalThis.databus.score); // 绘制当前分数

    // 游戏结束时停止帧循环并显示游戏结束画面
    if (globalThis.databus.isGameOver) {
      this.renderGameOver(ctx, globalThis.databus.score); // 绘制游戏结束画面
    }
  }
